    ...
    public ISchematic readFromNBT(final NBTTagCompound tagCompound) {
            ...
            for (int y = 0; y < height; y++) {
                for (int z = 0; z < length; z++) {
                    ...
                    try {
                        ...
                        logger.error("Could not set block state at {} to {} with metadata {}", pos, BLOCK_REGISTRY.getNameForObject(block), meta, e);
                    }
                }
            }
        }
        ...
        for (int i = 0; i < tileEntitiesList.tagCount(); i++) {
            try {
                ...
                logger.error("TileEntity failed to load properly!", e);
            }
        }
    ...
    public boolean writeToNBT(final NBTTagCompound tagCompound, final ISchematic schematic) {
        ...
        for (final TileEntity tileEntity : schematic.getTileEntities()) {
            try {
                ...
                if (--count > 0) {
                    ...
                    final Block block = blockState.getBlock();
                    logger.error("Block {}[{}] with TileEntity {} failed to save! Replacing with bedrock...", block, block != null ? BLOCK_REGISTRY.getNameForObject(block) : "?", tileEntity.getClass().getName(), e);
                }
        ...
        for (final Entity entity : entities) {
            try {
                ...
                logger.error("Entity {} failed to save, skipping!", entity, t);
            }
        }
